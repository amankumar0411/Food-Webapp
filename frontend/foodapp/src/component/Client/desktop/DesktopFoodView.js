...
function DesktopFoodView({
  ...
  favorites,
  toggleFavorite,
  onSwitchToQuickOrder,
  navigate,
  userNameDisplay,
  userInitials
}) {
  return (
    <div className="min-h-screen bg-oat-100 flex flex-col selection:bg-crimson selection:text-white">
      {/* BEGIN: MainHeader */}
      <header className="sticky top-0 z-40 bg-oat-100/95 backdrop-blur-md border-b border-sandborder transition-colors">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between gap-6">
          <div className="flex items-center gap-8 shrink-0">
            ...
            >
              <span className="text-2xl font-bold tracking-tighter uppercase font-serif text-crimson group-hover:text-crimson-dark transition-colors">ZAYKA</span>
      ...
      {/* BEGIN: MainContent */}
      <main className="flex-grow max-w-7xl mx-auto px-6 py-6 w-full space-y-10">
        {/* BEGIN: ModeSwitcherAndFilters */}
        <section className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-sandborder pb-6">
          <div className="inline-flex bg-oat-50 p-1.5 rounded-2xl border border-sandborder shadow-inner self-start md:self-auto">
            <button className="flex items-center gap-2.5 px-5 py-2 rounded-xl bg-white text-ink font-bold text-xs tracking-wider uppercase shadow-subtle border border-sandborder/50 transition-all cursor-pointer">
            ...
            <button 
              className="flex items-center gap-2 px-5 py-2 rounded-xl text-ink-muted hover:text-ink font-semibold text-xs tracking-wider uppercase transition-colors cursor-pointer"
              onClick={onSwitchToQuickOrder}
            >
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 custom-scrollbar">
            <button className="px-4 py-1.5 rounded-full bg-ink text-white text-xs font-bold tracking-wide shrink-0 shadow-sm cursor-pointer" onClick={() => navigate('/foodlistclient')}>All</button>
            <button className="px-3.5 py-1.5 rounded-full bg-white border border-sandborder hover:border-ink-soft text-ink-soft text-xs font-semibold tracking-wide shrink-0 transition-colors cursor-pointer" onClick={() => navigate('/foodlistclient')}>Fast Delivery (&lt;25 mins)</button>
          </div>
        </section>
        {/* END: ModeSwitcherAndFilters */}
...
export default DesktopFoodView;
